...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    switch (request.params.name) {
        ...
        case "copy_file": {
            ...
            try {
                ...
                return {
                    content: [{
                        type: "text",
                        text: `Copied file from ${source} to ${destination}`
                    }]
                };
            } catch (error: any) {
                return {
        ...
        case "get_directory_size": {
            ...
            async function getDirSize(dir: string): Promise<number> {
                ...
                const files = await Promise.all(dirents.map((dirent) => {
                    ...
                    return dirent.isDirectory() ? getDirSize(res) : fsp.stat(res).then(stat => stat.size);
                }));
                return files.flat(Infinity).reduce((i, size) => i + size, 0);
            }
            ...
            try {
                ...
                return {
                    content: [{
                        type: "text",
                        text: `Directory size: ${size} bytes`
                    }]
                };
            } catch (error: any) {
                return {
                    content: [{
                        type: "text",
                        text: `Error getting directory size: ${error.message}`
                    }],
                    isError: true
                }
            }
        ...
        case "create_symlink": {
            ...
            try {
                await fsp.symlink(source, destination);
                return {
                    content: [{
                        type: "text",
                        text: `Created symbolic link from ${source} to ${destination}`
                    }]
                };
            } catch (error: any) {
                return {
                    content: [{
                        type: "text",
                        text: `Error creating symbolic link: ${error.message}`
                    }],
                    isError: true
                }
            }
